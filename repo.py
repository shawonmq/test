...
#                    addon = re.search('addon.+?id="(.+?)".+?version="(.+?)"',data)
                    match =  re.compile('addon.+?id="(.+?)".+?version="(.+?)"', re.MULTILINE | re.DOTALL | re.IGNORECASE).findall(data)
                    if len(match) > 0:

                        for id, version in match:
                        	filename = str(id) + '-' + str(version) + '.zip'

                        	ziptargetpath = os.path.join( folder, filename)



                        	if (os.path.exists(ziptargetpath) == False):
                        	    print 'Creating zip file for ' +folder
                        	    zipdir(folder, filename)
                    '''
